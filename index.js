(function () {
  ...
  const gateRegister = document.getElementById('gateRegister');
  let logoutBtn = null;
  ...
  function reply(text) {
    ...
    if (
      ...
      return 'Xin chào, tôi là trợ lý CROWN. Tôi có thể hỗ trợ gì cho bạn?';
    }
    if (
      t.includes('xin chào') ||
      ...
      return 'Xin chào! Đây là CROWN. Tôi có thể giúp gì cho bạn hôm nay? 😊';
    }
    ...
    if (
      ...
      return 'Hello! Welcome to CROWN. How can I assist you today? 😊';
    }
    ...
    if (
      ...
      return `Kết nối với chúng tôi tại:\n` +
        `- SĐT/Zalo: 1900 1004\n` +
        `- Gmail: CROWNshop9999@gmail.com\n` +
        `- Facebook: https://www.facebook.com/CROWNshop/`;
    }
    ...
    if (
      ...
      return `Tất cả sản phẩm tại CROWN Shop đều được chế tác từ nguyên liệu cao cấp bậc nhất thế giới, gia công thủ công tinh xảo bởi các nghệ nhân hàng đầu. Chúng tôi không làm hàng đại trà.\n` +
    ...
    if (
      ...
      return `At CROWN Shop, every creation is forged exclusively from the finest materials the world has to offer, handcrafted with uncompromising artistry by the most skilled masters.\n` +
  ...
  function pushLoginHistory(email) {
    try {
      const key = userKey(email, 'login_history');
      const raw = localStorage.getItem(key);
      const arr = raw ? JSON.parse(raw) : [];
      const tz = (Intl.DateTimeFormat().resolvedOptions().timeZone || '');
      const ua = navigator.userAgent || '';
      arr.unshift({ at: Date.now(), tz, ua });
      localStorage.setItem(key, JSON.stringify(arr.slice(0, 20)));
    } catch (_) { }
  }
  function tooFrequentLogin(email) {
    try {
      const key = userKey(email, 'login_history');
      const raw = localStorage.getItem(key);
      const arr = raw ? JSON.parse(raw) : [];
      if (!arr.length) return false;
      const last = arr[0]?.at || 0;
  ...
  (function () {
    ...
    if (!cur && loginGate) {
      ...
      loginGate && loginGate.setAttribute('hidden', '');
      document.body.classList.remove('gate-active');
    }
  })();
  function closeLoginGate() {
    if (!loginGate) return;
    loginGate.classList.add('closing');
    setTimeout(() => {
      loginGate.setAttribute('hidden', '');
      loginGate.classList.remove('closing');
      document.body.classList.remove('gate-active');
    }, 320);
  }
  function isGateVisible() {
    return !!(loginGate && !loginGate.hasAttribute('hidden'));
  }
  ...
  function resumeGateIfNoUser() {
    if (!gateSuspended) return;
    if (!getCurrentUser()) {
      loginGate && loginGate.removeAttribute('hidden');
      document.body.classList.add('gate-active');
    }
    gateSuspended = false;
  }
  ...
  if (gateLogin && authModal) {
    gateLogin.addEventListener('click', () => {
      suspendGate();
      authTabs.forEach(t => t.classList.remove('active'));
      const tab = Array.from(authTabs).find(t => t.dataset.tab === 'login');
  ...
  if (gateRegister && authModal) {
    gateRegister.addEventListener('click', () => {
      suspendGate();
      authTabs.forEach(t => t.classList.remove('active'));
      const tab = Array.from(authTabs).find(t => t.dataset.tab === 'register');
      if (tab) tab.classList.add('active');
      loginForm.hidden = true;
      registerForm.hidden = false;
      toggleAuth(true);
      regEmail && regEmail.focus();
    });
  }
  ...
  if (loginForm && loginEmail && loginPass && loginMsg) {
    loginForm.addEventListener('submit', async e => {
      ...
      if (current && normalizeEmail(current) !== normalizeEmail(email)) {
        loginMsg.textContent = 'Vui lòng đăng xuất tài khoản hiện tại trước khi đăng nhập tài khoản khác.';
        if (loginSubmit) { loginSubmit.disabled = false; loginSubmit.textContent = 'Đăng nhập'; }
        return;
      }
      if (current && normalizeEmail(current) === normalizeEmail(email)) {
        loginMsg.textContent = 'Bạn đang đăng nhập tài khoản này.';
        if (loginSubmit) { loginSubmit.disabled = false; loginSubmit.textContent = 'Đăng nhập'; }
        return;
      }
      if (!isGmail(email)) {
      ...
      if (tooFrequentLogin(email)) {
        loginMsg.textContent = 'Bạn vừa đăng nhập gần đây. Vui lòng thử lại sau ít giây.';
        if (loginSubmit) { loginSubmit.disabled = false; loginSubmit.textContent = 'Đăng nhập'; }
        return;
      }
